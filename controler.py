...
class EmprestimoController:
    ...
    def __init__(self, emprestimos: List[e.Emprestimo] = None):
        # popula apenas uma vez
        if not self._emprestimos and emprestimos is None:
            self._seed()
        else:
    ...
    def criar(usuario, livro, dias=7):
...
class RelatorioController:
    ...
    relatorio: r.Relatorio = r.Relatorio([], [], [])

    def __init__(self, emprestimos: List[Dict], usuarios: List[u.Usuario], livros: List[l.Livro]):
        self.relatorio.set_livros(livros)
        self.relatorio.set_usuarios(usuarios)
        self.relatorio.set_emprestimos(emprestimos)

    def carregar_dados(self, emprestimos: List[Dict], usuarios: List[u.Usuario], livros: List[l.Livro]):
        self.relatorio.set_livros(livros)
        self.relatorio.set_usuarios(usuarios)
        self.relatorio.set_emprestimos(emprestimos)
    ...
    def livros_mais_emprestados(self, limite=10):
        contador = Counter(e["bookId"] for e in self.relatorio.get_emprestimos())
        mais_emprestados = contador.most_common(limite)
        # Retorna tuplas (Livro, quantidade)
        id_para_livro = {livro.id: livro for livro in self.livros}
        return [
            {"livro": id_para_livro.get(book_id), "quantidade": qtd}
            for book_id, qtd in mais_emprestados
            if book_id in id_para_livro
        ]

    # ----------------------------------------------------
    # 2. Usuários mais ativos (com mais empréstimos)
    # ----------------------------------------------------
    def usuarios_mais_ativos(self, limite=10):
        contador = Counter(e.usuario.nome for e in self.relatorio.get_emprestimos())
        mais_ativos = contador.most_common(limite)
        id_para_usuario = {u.nome: u for u in self.relatorio.get_usuarios()}
        return [
            {"usuario": id_para_usuario.get(user_nome), "quantidade": qtd}
            for user_nome, qtd in mais_ativos
            if user_nome in id_para_usuario
        ]

    # ----------------------------------------------------
    # 3. Livros por categoria
    # (se houver atributo 'categoria' em Livro, senão retorna genérico)
    # ----------------------------------------------------
    def livros_por_categoria(self):
        categorias = defaultdict(list)
        for livro in self.relatorio.get_livros():
            categoria = getattr(livro, "categoria", "Desconhecida")
            categorias[categoria].append(livro)
        return {cat: len(lst) for cat, lst in categorias.items()}

    # ----------------------------------------------------
    # 4. Taxa de ocupação (livros emprestados / total)
    # ----------------------------------------------------
    def taxa_ocupacao(self):
        total = len(self.relatorio.get_livros())
        if total == 0:
            return 0.0
        emprestados = sum(1 for l in self.relatorio.get_livros() if getattr(l, "status", None) == "emprestado")
        return round(emprestados / total, 2)

    # ----------------------------------------------------
    # 5. Empréstimos por período
    # ----------------------------------------------------
    def emprestimos_por_periodo(self, data_inicio, data_fim):
        dt_inicio = datetime.fromisoformat(data_inicio)
        dt_fim = datetime.fromisoformat(data_fim)
        return [
            e for e in self.relatorio.get_emprestimos()
            if dt_inicio <= datetime.fromisoformat(e["loanDate"]) <= dt_fim
        ]

    # ----------------------------------------------------
    # 6. Total de empréstimos ativos
    # ----------------------------------------------------
    def total_emprestimos_ativos(self):
        return sum(1 for e in self.relatorio.get_emprestimos() if e["status"] == "ACTIVE")

    # ----------------------------------------------------
    # 7. Empréstimos em atraso
    # ----------------------------------------------------
    def emprestimos_em_atraso(self):
        agora = datetime.now()
        return [
            e for e in self.relatorio.get_emprestimos()
            if e["status"] == "ACTIVE" and datetime.fromisoformat(e["dueDate"]) < agora
        ]